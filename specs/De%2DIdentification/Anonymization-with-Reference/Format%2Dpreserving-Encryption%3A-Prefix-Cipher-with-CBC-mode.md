...
**Concern** Since we are transforming Id characters based on a fixed table, same characters will be transfromed to another same character. For example, with a code book generated by AES like
```
"a" => "f",
"b" => "c",
"c" => "e",
...
...
```
Provided plain text "abcea" , we can get cipher text "fcebf" The text pattern will still exist in cipher text.
...
For example, with a code book like:
```
"a" => "f",
"b" => "c",
"c" => "e",
...
"W" => "d"
```
Provided plain text "abcea" without an IV, we can get the first block result 'f', and add to the second block input, we get (index('f')+index('b')) % radix = 6, then the second input is 'W', we get 'd' as cipher text,... at last, the cipher would be "fdWbc", the plain text pattern has been eliminated in cipher text.

Here is a straightforward graphic of CBC.
![image.png](/.attachments/image-cea7d175-100f-435a-86ca-6a0d01f5554d.png)

# Analysis
With CBC mode combined with prefix cipher, attacks from character statistics will not work. There might be brute force attack to the code book. 
As AES is a trustable algorithm, the code book generated by prefix cipher can be viewed as random. The cost of brute force attack to the code book is O(64!).
