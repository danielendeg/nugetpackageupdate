...
| As a Service administrator, I can request a specific region (different from Azure paired regions) for the seconary data replica for Azure API for FHIR                     | Specify an Azure region in the SR support ticket if it is different from Azure paired regions. Explain why the specific region is required and if it is a hard requirement. Note that the region must meet data residency in Azure.  | P0       |
