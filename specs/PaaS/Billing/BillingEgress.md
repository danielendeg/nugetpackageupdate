...
1. An internal source http://168.63.129.16/machine/plugins?comp=nmagent&type=GetIpRangesV2 that comes from an internal rollout. The data format is xml and a snippet for Australia Central 2 is below. We will use only the public ranges as they are only ones exposed to customers.
````xml
