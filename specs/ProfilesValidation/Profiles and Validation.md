...
There are a few things to consider around referential integrity. Specifically, it should not be possible for a user to probe if a certain patient exists by adding an observation with a reference to that patient and if it succeeds, the patient exists. Similarly `$validate` could be used to probe if certain resources exist.
